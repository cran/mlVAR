...
# 
# stan_mlVAR <- 
#   function(model,augData,idvar,verbose=TRUE,temporal="correlated",contemporaneous = "correlated",nCores=1,...){
# 
# 
#     
...
#     
#     # Unique model:
#     uniqueModel <- model %>% group_by_("pred","lag") %>% summarize_(predID = ~unique(predID))
#     
#   
#     # Stan ID:
#     augData[["STANID"]] <- as.numeric(as.factor(augData[[idvar]]))
#     
#     # ID translation:
#     stanCode <- eval(substitute({
#       augData %>% 
#         dplyr::group_by(idvar) %>% dplyr::summarize( 
#                                        STANID = unique(STANID))
#       }, list(idvar = as.name(idvar))))
# 
#     
#     # Number of randoms: nVar + nVar^2 * nLag
#     
#     ### Create data list ###
#     stanData <- list(
#       Y = augData[,Outcomes],
#       nY = length(Outcomes),
#       nSample = nrow(augData),
#       ID = augData$STANID,
#       nCluster = nrow(stanCode),
#       X = augData[,uniqueModel$predID],
#       nX = nrow(uniqueModel),
#       nRandom = nVar + nVar^2 * nLag,
#       Beta_ix = matrix(seq(nVar+1,nVar + nVar^2 * nLag),nVar, nVar*nLag)
#     )
#     
#     ### RUN STAN ###
#     # For now load package:
#     library("rstan")
#     options(mc.cores = min(nCores,parallel::detectCores()))
#     
#     fit <- rstan::stan(model_code = STANMODEL, data = stanData, ...)
#     
#     
#     # Output list:
#     lmerResults <- list()
